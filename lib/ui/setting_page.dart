  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          ...
          title: const Text('iSee Settings'),
        ),
        body: Container(
        ...
        children: <CardSettingsSection>[
          ...
          CardSettingsSection(
            ...
            children: <CardSettingsWidget>[
              ...
              CardSettingsText(
                key: const Key('liveApiUrl'),
                label: 'Live API URL',
                maxLength: 256,
                initialValue: DB.setting.liveApiUrl,
                validator: (newUrl) {
                  if (newUrl == null || !newUrl.startsWith('http')) {
                    return 'Must be a valid url.';
                  } else {
                ...
                onChanged: (newUrl) {
                  Log.log.fine('saving liveApiUrl: $newUrl');
                  DB.setting
                      .changeSetting(SettingKeyConstants.liveApiUrl, newUrl);
                },
              ),
              CardSettingsText(
                  key: const Key('liveWsUrl'),
                  label: 'Live Ws URL',
                  maxLength: 256,
                  initialValue: DB.setting.liveWsUrl,
                  validator: (newUrl) {
                    if (newUrl == null ||
                        !(newUrl.startsWith('ws') ||
                            newUrl.startsWith('wss'))) {
                      return 'Must be a valid ws url.';
                    } else {
                      return null;
                    }
                  },
                  onChanged: (newUrl) {
                    Log.log.fine('saving liveWsUrl: $newUrl');
                    DB.setting
                        .changeSetting(SettingKeyConstants.liveWsUrl, newUrl);
                  }),
            ],
          ),
        ],
