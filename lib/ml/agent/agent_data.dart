  ...
  factory TaskInfo.fromJson(Map<String, dynamic> json) {
    ...
    try {
      if (json['keyword'] != null) {
        keyword = json['keyword'].toString().split(',')[0];
      }
    } catch (e) {
      Log.log.info('TaskInfo.fromJson $e');
    }
...
class AgentData {
  ...
  Future<SeeGoal?> getGoalFromDbById(int goalId) async {
    SeeGoal? goal = await DB.isar?.seeGoals.get(goalId);
    if (goal == null) {
      Log.log.warning('getGoalFromDbById, id not found: $goalId');
      return null;
    }
    return goal;
  }
  ...
  Future<SeeTask?> getTaskFromDbById(int taskId) async {
    SeeTask? task = await DB.isar?.seeTasks.get(taskId);
    if (task == null) {
      Log.log.info('getTaskFromDbById: task not found $taskId');
    }
    return task;
  }
      ...
      await DB.isar?.writeTxn(() async {
        if (quizInfo.id > 0) {
          ...
          if (oldQuiz != null) {
            ...
            if (score > 0) {
              oldQuiz.score = score;
              oldQuiz.historyScores = [...oldQuiz.historyScores, score];
            }
            if (feedback != null) {
              oldQuiz.feedback = feedback;
            }
            if (discard || nextReviewTime == null) {
